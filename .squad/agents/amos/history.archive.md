...
### 2026-05-09T04:57:53Z — Round 9: ARM artifact cleanup + gitignore fix (amos-8 & amos-9)

**amos-8 — Remove tenantId parameter, derive from subscription()  
Commit:** 523f3d0

- Removed `param tenantId string` from `infra/main.bicep`
- Added `var tenantId = subscription().tenantId` (available at deployment time from active context)
- Removed `tenantId` from `infra/main.parameters.json` (eliminates AZD prompt)
- Module contract with `infra/modules/appService.bicep` unchanged
...
- Rationale: ARM JSON is auto-generated from Bicep; Bicep is the canonical source
- Verification: `bicep build infra/main.bicep` → exit 0, no errors (clean build)

**Decision logged:** D14

**Combined impact:**
- Bicep compilation gate passing (exit 0)
- AZD provisioning no longer prompts for tenantId
- Repository cleaner; ARM JSON removed from version control
- Source-of-truth clarified: Bicep source only, not auto-generated JSON
