...
def _run_oauth_flow(
    ...
    with httpx.Client(timeout=30.0, follow_redirects=True, http2=False) as client:
        ...
        if open_browser:
            ...
            click.echo(
                "ℹ️ Browser launch disabled; visit the URL above manually.", err=True
            )
...
def run_flow(
    ...
    access_token = _run_oauth_flow(
        mode, server_url, client_id, audience, scope, timeout, open_browser
    )
